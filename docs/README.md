...
当前基线：Day 11 已完成。服务端用户出价 API、Redis Lua 原子出价、幂等、封顶成交、防狙击延时、Socket.IO 房间隔离、断线重连 snapshot、outbox 广播发布、移动端真实 REST / Socket.IO 联动、主播端创建商品 / 配置竞拍 / 启动 / 取消 / 查看订单闭环已落地；Day 11 已补服务级 e2e 异常场景覆盖；正式压测仍为后续目标。

## 必读文档

| 文档 | 用途 | 当前状态 |
| --- | --- | --- |
| `progress.md` | Day 1-Day 11 进度、Day 12 下一步 | 持续维护 |
...
| 文档 | 用途 | 当前状态 |
| --- | --- | --- |
| `manual-test.md` | 手工测试清单和执行记录 | Day 11 已补服务级异常场景 e2e；真实浏览器多窗口仍待手测 |
| `performance-report.md` | 压测环境、结果和一致性校验记录 | 有单元级并发和服务级 e2e 记录，暂无真实压测数据 |
| `demo-script.md` | 最终录屏和答辩演示脚本 | 包含 Day 11 后台创建、移动端出价和异常场景可演示范围 |
