...
- 同一竞拍的幂等检查、Redis Lua 和 DB 持久化先进入当前进程内 `auctionId` 级队列，再获取 Redis 分布式锁 `auction:{auctionId}:bid_lock`，保证多实例下失败 accepted bid 可先回滚，再处理后续请求。
